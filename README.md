...
`config.txt` 可调整运行参数。推荐输入 `/config` 打开交互式配置面板，用键盘上下左右键修改并即时生效；也可手动编辑文件后输入 `/reload`：

| 参数 | 默认值 | 说明 |
|------|--------|------|
| 最大记忆轮数 | 50 | 超出后自动丢弃早期对话 |
| 最大工具调用轮数 | 70 | 单次对话最多调用工具次数 |
| 连接超时秒数 | 10 | API 连接超时 |
| 响应超时秒数 | 120 | API 响应超时 |
